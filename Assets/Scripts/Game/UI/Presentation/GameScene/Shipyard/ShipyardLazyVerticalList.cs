...
public sealed class ShipyardLazyVerticalList : MonoBehaviour
{
    ...
    /// <summary>
    /// 编队选舰：点击槽位时回调 cardId（非编队模式应为 null）。
    /// </summary>
    private Action<string> _onFleetPickSlotClicked;

    /// <summary>
    /// 船坞模式：点击槽位打开舰娘详情时的回调（与编队模式互斥，编队优先）。
    /// </summary>
    private Action<string> _onShipyardDetailClicked;
    ...
    /// <summary>
    /// RectTransform 尺寸变化时检测宽度变化并重建网格（替代每帧 LateUpdate 轮询）。
    /// </summary>
    ...
    /// <summary>
    /// 编队选舰模式下为每个槽位绑定点击（船坞模式传入 null 取消监听）。
    /// </summary>
    /// <param name="handler">点击槽位时传入对应 cardId。</param>
    public void SetFleetPickClickHandler(Action<string> handler)
    {
        _onFleetPickSlotClicked = handler;
    }

    /// <summary>
    /// 船坞模式下点击槽位打开详情（传入 null 则关闭槽位 Button 交互）。
    /// </summary>
    /// <param name="handler">点击时传入 cardId。</param>
    public void SetShipyardDetailClickHandler(Action<string> handler)
    {
        _onShipyardDetailClicked = handler;
    }

    /// <summary>
    /// 网格内容根节点（编队选舰刷新遮罩时遍历子节点）。
    /// </summary>
    ...
    /// <summary>
    /// 为槽位挂载或刷新 Button：编队选舰优先；否则船坞详情点击；无回调则关闭交互。
    /// </summary>
    /// <param name="rt">槽位根 RectTransform。</param>
    /// <param name="entry">收藏条目。</param>
    private void WireSlotInteraction(RectTransform rt, CardCollectionStore.CardEntry entry)
    {
        if (rt == null)
        {
            return;
        ...
        var cardId = entry != null && !string.IsNullOrWhiteSpace(entry.cardId)
            ? entry.cardId.Trim()
            : string.Empty;

        if (string.IsNullOrEmpty(cardId))
        {
            DisableSlotButton(btn);
            return;
        }

        if (_onFleetPickSlotClicked != null)
        {
            EnsureSlotButton(rt, ref btn);
            btn.onClick.RemoveAllListeners();
            var capturedFleet = cardId;
            btn.onClick.AddListener(() => _onFleetPickSlotClicked.Invoke(capturedFleet));
            return;
        }

        if (_onShipyardDetailClicked != null)
        {
            EnsureSlotButton(rt, ref btn);
            btn.onClick.RemoveAllListeners();
            var capturedDetail = cardId;
            btn.onClick.AddListener(() => _onShipyardDetailClicked.Invoke(capturedDetail));
            return;
        }
    ...
    /// 无交互需求时禁用槽位按钮。
    /// </summary>
    private static void DisableSlotButton(Button btn)
    {
        if (btn == null)
        {
            return;
    ...
    /// <summary>
    /// 保证槽位存在可点击 Button 与目标 Graphic。
    /// </summary>
    private static void EnsureSlotButton(RectTransform rt, ref Button btn)
    {
        if (btn == null)
        {
        ...
        var graphic = rt.GetComponent<Image>() ?? rt.GetComponentInChildren<Image>(true);
