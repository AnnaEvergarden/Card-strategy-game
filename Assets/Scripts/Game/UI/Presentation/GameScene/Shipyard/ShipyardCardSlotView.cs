...
public sealed class ShipyardCardSlotView : MonoBehaviour
{
    ...
    /// <summary>
    /// 编队选舰序号遮罩根节点（可选；未指定时在首次需要时创建）。
    /// </summary>
    [SerializeField] private RectTransform fleetPickMaskRoot;

    /// <summary>
    /// 编队选舰序号文本（可选）。
    /// </summary>
    [SerializeField] private TMP_Text fleetPickOrderText;

    /// <summary>
    /// 遮罩半透明底色。
    /// </summary>
    [SerializeField] private Color fleetPickMaskColor = new(0f, 0f, 0f, 0.45f);
    ...
    {
        BoundCardId = entry != null && !string.IsNullOrWhiteSpace(entry.cardId)
            ? entry.cardId.Trim()
            : string.Empty;
    ...
    /// <summary>
    /// 编队选舰：更新上阵序号遮罩（0 表示未选中，隐藏遮罩）。
    /// </summary>
    /// <param name="orderOneBased">上阵顺序 1～6；0 为隐藏。</param>
    public void SetFleetPickSelectionOrder(int orderOneBased)
    {
        if (orderOneBased <= 0)
        {
            if (fleetPickMaskRoot != null)
            {
                fleetPickMaskRoot.gameObject.SetActive(false);
            }

            return;
        }

        EnsureFleetPickMarkUi();
        if (fleetPickMaskRoot != null)
        {
            fleetPickMaskRoot.gameObject.SetActive(true);
        }

        if (fleetPickOrderText != null)
        {
            fleetPickOrderText.text = orderOneBased.ToString();
        }
    }
    ...
    /// <summary>
    /// 确保编队遮罩 UI 存在（铺满槽位根 Rect，不拦截卡片点击，射线穿透由上层 Button 处理）。
    /// </summary>
    private void EnsureFleetPickMarkUi()
    {
        ...
        if (fleetPickMaskRoot == null)
        {
            var go = new GameObject("FleetPickMark", typeof(RectTransform), typeof(Image));
            ...
            img.color = fleetPickMaskColor;
            img.raycastTarget = false;
            fleetPickMaskRoot = rt;
            fleetPickMaskRoot.gameObject.SetActive(false);
        }

        if (fleetPickOrderText == null && fleetPickMaskRoot != null)
        {
            var textGo = new GameObject("OrderText", typeof(RectTransform));
            var tr = textGo.GetComponent<RectTransform>();
            tr.SetParent(fleetPickMaskRoot, false);
